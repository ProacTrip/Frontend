...
//Manejamos el cierre de sesión
  ...
  const navLinks = [
    ...
    { href: '/home/ofertas', label: 'Ofertas' },
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* NAVBAR */}
      <nav className="bg-[#FF6B6B] shadow-lg">
            ...
            <div className="hidden md:flex items-center gap-8">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-white hover:text-gray-200 transition-colors font-medium uppercase text-sm"
                >
                  {link.label}
                </Link>
              ))}
