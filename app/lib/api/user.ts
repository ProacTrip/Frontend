...
/**
 * List available default avatar options.
 *
 * Direct fetch with credentials:"include".
 * 10s timeout via AbortController.
 */
export async function listDefaultAvatars(): Promise<DefaultAvatar[]> {
  const endpoint = '/v1/user/profile/avatars/default';
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 10000);
...
/**
 * Select a default avatar by name.
 *
 * Direct fetch with credentials:"include".
 * 10s timeout via AbortController.
 */
export async function selectDefaultAvatar(avatar_name: string): Promise<string> {
  const endpoint = '/v1/user/profile/avatar/default';
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 10000);
