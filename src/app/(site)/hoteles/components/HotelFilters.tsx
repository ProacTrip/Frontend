...
export default function HotelFilters({
  ...
  return (
    <>
      ...
      <div
        ref={filterBarRef}
        className="fixed top-[72px] left-0 right-0 z-40 bg-white border-b border-[#e8e8e8] flex items-stretch px-4 lg:px-8 h-14 lg:h-16"
      >
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between py-3 border-b border-[#e8e8e8]">
                  ...
                  <div className="flex items-center gap-4">
                    <button
                      onClick={() => apply({ bedrooms: Math.max(0, (filters.bedrooms ?? 0) - 1) })}
                    ...
                    <span className="text-sm font-medium min-w-[20px] text-center">{filters.bedrooms ?? 0}</span>
                    <button
                      onClick={() => apply({ bedrooms: (filters.bedrooms ?? 0) + 1 })}
                ...
                <div className="flex items-center justify-between py-3">
                  ...
                  <div className="flex items-center gap-4">
                    <button
                      onClick={() => apply({ bathrooms: Math.max(0, (filters.bathrooms ?? 0) - 1) })}
                    ...
                    <span className="text-sm font-medium min-w-[20px] text-center">{filters.bathrooms ?? 0}</span>
                    <button
                      onClick={() => apply({ bathrooms: (filters.bathrooms ?? 0) + 1 })}
                      className="w-11 h-11 rounded-full border-[1.5px] border-[#e8e8e8] bg-white flex items-center justify-center text-[#111] hover:border-[#888] transition-colors"
        ...
        {/* ── MOBILE: Compact filter button (hidden on desktop) ── */}
        <button
          onClick={onOpenModal}
          className={`lg:hidden relative flex items-center gap-2 px-4 py-2 rounded-full border text-[13.5px] font-medium transition-all whitespace-nowrap shrink-0 cursor-pointer ${
            totalActive > 0
              ? 'border-[#111] border-2 text-[#111]'
              : 'border-[#e8e8e8] text-[#111] hover:border-[#aaa] hover:shadow-[0_2px_8px_rgba(0,0,0,0.08)]'
          }`}
        >
          ...
          Filtros
          {totalActive > 0 && (
            <span className="ml-1 min-w-[18px] h-[18px] rounded-full bg-[#111] text-white text-[11px] font-bold flex items-center justify-center px-1">
              {totalActive}
            </span>
          )}
        </button>

        {/* ── MOBILE: Clear button (hidden on desktop) ── */}
        {totalActive > 0 && (
          <button
            onClick={() =>
              onFilterChange(
            ...
            className="lg:hidden shrink-0 flex items-center gap-1 px-3 py-2 rounded-full text-[13px] font-medium text-[#6A7282] hover:text-[#111] hover:bg-[#f5f5f5] transition-colors cursor-pointer"
          >
            <X className="w-3.5 h-3.5" />
            Limpiar
          </button>
