...
import {
  X, Plane, Clock, Luggage, Leaf, AlertCircle,
  Info, Check, Wifi, LogIn,
} from 'lucide-react';
  ...
  return (
    <Dialog open={isOpen} onClose={onClose} className="relative z-50">
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4 sm:p-6">
        <DialogPanel className="relative w-full max-w-3xl max-h-[90vh] bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col">
          {/* HEADER */}
          <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-100 bg-neutral-50">
            <div>
              <DialogTitle className="text-lg font-bold text-neutral-900">Detalles del vuelo</DialogTitle>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div className="bg-neutral-50 rounded-xl p-6 border border-neutral-200">
              <div className="flex items-center justify-between">
                <div className="text-center">
                  <div className="text-3xl font-bold text-neutral-900">{formatTime(legs[0]?.origin?.datetime)}</div>
                  <div className="text-lg font-semibold text-neutral-900 mt-1">{legs[0]?.origin?.code}</div>
                  <div className="text-sm text-neutral-600 mt-1">{legs[0]?.origin?.city}</div>
                </div>

                <div className="flex-1 px-8 flex flex-col items-center">
                  <div className="text-sm text-neutral-500 font-medium mb-2">{formatDuration(segment.totalDuration)}</div>
                  <div className="w-full flex items-center gap-2">
                    <div className="h-0.5 flex-1 bg-neutral-300" />
                    <Plane className="w-6 h-6 text-neutral-900 rotate-90" />
                    <div className="h-0.5 flex-1 bg-neutral-300" />
                  </div>
                ...
                <div className="text-center">
                  <div className="text-3xl font-bold text-neutral-900">{formatTime(legs[legs.length - 1]?.destination?.datetime)}</div>
                  <div className="text-lg font-semibold text-neutral-900 mt-1">{legs[legs.length - 1]?.destination?.code}</div>
                  <div className="text-sm text-neutral-600 mt-1">{legs[legs.length - 1]?.destination?.city}</div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <h3 className="font-bold text-neutral-900 flex items-center gap-2">
                <Plane className="w-5 h-5 text-neutral-900" /> Itinerario detallado
              </h3>

              {legs.map((leg, index) => (
                <div key={`leg-${index}`} className="relative">
                  {index < legs.length - 1 && (
                    <div className="absolute left-6 top-16 bottom-0 w-0.5 bg-neutral-200 h-full" />
                  )}

                  <div className="flex gap-4">
                    <div className="flex flex-col items-center w-12 shrink-0">
                      <div className="w-3 h-3 bg-neutral-900 rounded-full ring-4 ring-neutral-900/20" />
                      {index < legs.length - 1 && <div className="flex-1 w-0.5 bg-neutral-200 my-2" />}
                    </div>
                    ...
                    <div className="flex-1 pb-8">
                      <div className="bg-white border border-neutral-200 rounded-xl p-4 hover:shadow-md transition-shadow">
                        <div className="flex justify-between items-start mb-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="font-semibold text-neutral-900">{leg.flightNumber}</div>
                          ...
                          <div className="text-right">
                            <div className="text-sm font-medium text-neutral-900">{formatDuration(leg.duration)}</div>
                            <div className="text-xs text-neutral-500">{leg.aircraft}</div>
                          </div>
                        </div>

                        <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-neutral-100">
                          <div>
                            <div className="text-xs text-neutral-500 uppercase tracking-wide">Salida</div>
                            <div className="font-bold text-neutral-900">{formatTime(leg.origin?.datetime)}</div>
                            <div className="text-sm text-neutral-700">{leg.origin?.name}</div>
                            <div className="text-xs text-neutral-500 mt-1">
                              Terminal {leg.origin?.terminal || '--'}
                            </div>
                          </div>
                          <div className="text-right">
                            <div className="text-xs text-neutral-500 uppercase tracking-wide">Llegada</div>
                            <div className="font-bold text-neutral-900">{formatTime(leg.destination?.datetime)}</div>
                            <div className="text-sm text-neutral-700">{leg.destination?.name}</div>
                            <div className="text-xs text-neutral-500 mt-1">
                        ...
                        <div className="flex gap-4 mt-4 pt-4 border-t border-neutral-100 text-sm">
                      ...
                      {index < legs.length - 1 && layovers[index] && (
                        <div className="mt-4 ml-4 p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-3">
            ...
            <div className="bg-neutral-50 rounded-xl p-4 border border-neutral-200">
              <h4 className="font-semibold text-neutral-900 flex items-center gap-2 mb-3">
                <Luggage className="w-5 h-5 text-neutral-600" /> Equipaje incluido
              </h4>
            ...
            {offer.carbonEmissions && (
              <div className={`rounded-xl p-4 border ${
                offer.carbonEmissions.differencePercent < 0 ? 'bg-green-50 border-green-200' : 'bg-neutral-50 border-neutral-200'
              }`}>
                <div className="flex items-start gap-3">
                  <Leaf className={`w-5 h-5 shrink-0 ${
                    offer.carbonEmissions.differencePercent < 0 ? 'text-green-600' : 'text-neutral-500'
                  }`} />
                  <div>
                    <h4 className={`font-semibold text-sm ${
                      offer.carbonEmissions.differencePercent < 0 ? 'text-green-900' : 'text-neutral-900'
                    }`}>Emisiones de CO₂</h4>
          ...
          <div className="border-t border-neutral-200 p-6 bg-neutral-50">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <div className="text-sm text-neutral-500">Precio total</div>
              </div>
              {isAuthenticated ? (
                <button
                  onClick={() => { onSelect?.(offer); onClose(); }}
                  className="px-8 py-3 bg-neutral-900 text-white font-semibold rounded-full hover:bg-neutral-800 transition-colors"
                >
                  Continuar
                </button>
              ) : (
                <button
                  ...
                  className="px-8 py-3 bg-neutral-900 text-white font-semibold rounded-full hover:bg-neutral-800 transition-colors flex items-center gap-2"
                >
